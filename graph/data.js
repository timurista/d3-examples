...
function getSent(val) {
  if (val > 0) return "Positive";
  if (val < 0) return "Negative";
  if (val === 0) return "Neutral";
}

var chart = sent.map(x => ({
  ...x,
  sentiment: getSent(x.value),
  normalizedSent: Math.sign(x.value)
}));
...
var sentVals = sent.map(x => x.value);
var total = sentVals.length;
var mean = sentVals.reduce((a, b) => a + b) / total;
var variance = sentVals.reduce((a, b) => a + (b - mean) ** 2, 0);
var std = Math.sqrt(variance / total);
console.log(sentVals);
console.log(total, mean, variance, std);
var zScore = val => (val - mean) / std;
chart = chart.map(x => ({ ...x, zScore: zScore(x.value) }));

var positives = chart.filter(x => x.normalizedSent === 1);
var negatives = chart.filter(x => x.normalizedSent === -1);
var neutrals = chart.filter(x => x.normalizedSent === 0);
